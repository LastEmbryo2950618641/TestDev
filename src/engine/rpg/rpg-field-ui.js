...
window.GameModules.rpgFieldUi = {
  ...
  rpgItemPrerequisiteTags(item = {}) {
    const tags = [];
    if ((item.requiredSkills || []).length) tags.push(`技能x${item.requiredSkills.length}`);
    if ((item.requiredKnowledge || []).length) tags.push(`知识x${item.requiredKnowledge.length}`);
    if ((item.requiredIntrinsicBase || []).length) tags.push(`身体x${item.requiredIntrinsicBase.length}`);
    return tags;
  },
  ...
  rpgFieldSummary(field) {
    ...
    const unit = { knowledge: '条', skills: '条', professions: '项', bodyProfile: '部位', dressedProfile: '部位', bodyStatus: '部位', sexualExperienceParts: '类', sexualPartners: '人' }[field.key] || '项';
  ...
  roleCardReasonGetter(profile = {}) {
    ...
    const wrongSubject = (text, label = '') => {
      ...
      return /(作为|是|属于|承担|体现).{0,18}(妹妹|姐姐|哥哥|弟弟|父亲|母亲|女儿|儿子)/.test(value)
        || /(妹妹|姐姐|哥哥|弟弟|父亲|母亲|女儿|儿子).{0,12}(身份|性格|外貌|生日|职业|资料)/.test(value);
    };
    const usable = (text, label) => !/^错误[:：]?\s*缺少AI给出的变化原因/.test(String(text || '').trim()) && !wrongSubject(text, label) && String(text || '').trim();
  ...
  defaultIntimacyBodyFields(state = {}) {
    ...
    const sexRows = Object.entries(template?.sexPartLabels || {}).map(([partKey, name]) => ({
      partKey,
      name,
      count: 0,
      initialCount: 0,
      laterCount: 0,
      prompt: template?.sexPartPrompts?.[partKey] || template?.sexPartPrompts?.other || '',
      type: template?.fieldMeta?.sexualExperienceParts?.kind || '性经验分类',
      pendingAiInit: true,
    }));
    const bodyRows = Object.values(bodyStatus || {}).map((item) => ({
      ...item,
      name: item.part || item.partKey,
      type: template?.fieldMeta?.bodyStatus?.kind || '当前身体状态',
      pendingAiInit: true,
      reason: '尚未经过现实推演 AI 初始化；当前仅按模板占位显示。',
    }));
    const base = {
      templateKey: 'intimacyBody',
      stateId: state.id || '',
      worldTag: state.worldTag || state.profile?.work || '原创世界',
      targetType: state.profile?.isPlayer ? '非角色' : '角色',
      commonField: true,
      pendingAiInit: true,
      reason: '待 AI 初始化。',
    };
    ...
    return [
      {
        ...
        ...base,
        ...(meta.sexualStatus || {}),
        value: `${intimacy.sexualStatus || '待AI判断'}（模板占位，待AI初始化）`,
        raw: intimacy.sexualStatus || '待AI判断',
      },
      {
        key: 'sexualPartnerCount',
        ...base,
        ...(meta.sexualPartnerCount || {}),
        value: `${Number(intimacy.sexualPartnerCount) || 0}人（模板占位，待AI初始化）`,
        raw: Number(intimacy.sexualPartnerCount) || 0,
      },
      {
        key: 'sexualPartners',
        ...base,
        ...(meta.sexualPartners || {}),
        value: [template?.displayTexts?.noPartner || '无', '模板占位，待AI初始化'],
        raw: [template?.displayTexts?.noPartner || '无'],
      },
      {
        key: 'sexualExperienceCount',
        ...base,
        ...(meta.sexualExperienceCount || {}),
        value: `${Number(intimacy.sexualExperienceCount) || 0}次（模板占位，待AI初始化）`,
        raw: Number(intimacy.sexualExperienceCount) || 0,
      },
      {
        key: 'sexualExperienceParts',
        ...base,
        ...(meta.sexualExperienceParts || {}),
        value: sexRows.map((item) => `${item.name}: 0(初次见面) + 0(后续次数)`),
        raw: sexRows,
      },
      {
        key: 'bodyStatus',
        ...base,
        ...(meta.bodyStatus || {}),
        value: bodyRows.map((item) => template?.formatBodyStatus?.(item) || `${item.part || item.partKey}: ${item.status || '--'}`),
        raw: bodyRows,
        desc: '身体状态尚未经过现实推演 AI 初始化；当前显示的是模板占位，不作为真实初始值。',
      },
    ];
  },
  ...
  profileLongingField(state = {}) {
    ...
    return {
      key: 'longing_to_player',
      stateId: state.id,
      label: '思念度',
      kind: '关系状态',
      value: `${value.toFixed(1)}/100`,
      raw: value,
      desc: '角色对玩家的思念累计值，达到 100 时会在现实推演中触发一次思念事件。',
      reason: `${p.name || state.name || '该角色'}的思念度由现实推演间隔时间、好感度与随机系数共同结算累积；最近结算：${time}。`,
      worldTag: p.work || state.worldTag || '原创世界',
      targetType: '角色',
      commonField: true,
    };
  },
  ...
  profileDressedStateField(state = {}) {
    ...
    const field = this.profileBodyStateField(state, source.dressedProfile, {
      ...
      kind: '盛装状态',
      type: '盛装状态',
      desc: '角色盛装或打扮完成后各身体部位的造型、修饰与衣物包裹状态。',
  ...
  profileBodyStateField(state = {}, source = [], meta = {}) {
    ...
    if (meta.metaText) valueRows.unshift(`銆愬叏灞€、{meta.metaText}`);
    return {
  ...
  usableChangeReason(reason, blocked = []) {
    ...
    if (/^错误[:：]?\s*缺少AI给出的变化原因/.test(text)) return '';
    if (/^(AI演算|系统结算|系统词条调整|用户主动)$/.test(text)) return '';
    if (/词条说明|当前作用|用于记录|暂无详细说明/.test(text)) return '';
    if (/依据.*(当前值|上限|经验曲线)|当前.*依据|被记录为当前|后续(获得|使用|转让|遗失|损坏|穿戴|状态变化)/.test(text)) return '';
  ...
  fallbackBasis(field, obj = null, kind = '', name = '') {
    ...
    if (obj && finalKind === '穿着') return `${profile.name || '该人物'}当前穿着 ${finalName}，后续只有在明确换装、脱下、破损或洗浴等事件中才会更新。`;
    if (obj) return `${profile.name || '该人物'}持有 ${finalName}，这是其 ${finalKind}、当前处境或既有生活经历的一部分，后续会随明确剧情事件更新。`;
    if (field?.source) return `${profile.name || '该人物'}的 ${finalName} 由初始经历、等级成长、自由分配和非玩家成长共同形成。`;
    return `${profile.name || '该人物'}的 ${finalName} 按其当前身份、处境、过往经历和可支配资源固化。`;
  },
  ...
  cleanDetailText(text = '') {
    return String(text || '').replace(/([：])(?=(濡瑰|姐姐|鍝ュ摜|寮熷紵|鐖朵翰|姣嶄翰|鍏勯暱|朋友|鍚屽|鍚屼簨)[：'])/g, '：');
  },

  pollutedDetailText(text = '') {
    const value = String(text || '').trim();
    return value.length > 90 || /鍙樺寲鏂瑰紡|生成来源|词条名AI生成|鍊糀I生成/.test(value) || /[：](濡瑰|姐姐|鍝ュ摜|寮熷紵|鐖朵翰|姣嶄翰|鍏勯暱)[：]/.test(value);
  },
  ...
  learnedDefinition(kind, name, obj = {}, lexicon = null, info = {}) {
    const explicit = [info.description, lexicon?.description, lexicon?.summary, obj.description, obj.desc, obj.source]
      .find((x) => x && !/暂无|资料|当前作用/.test(String(x)));
    ...
    const typeText = {
      '知识': `对“${name}”这一知识领域的概念、规则、背景和应用范围的理解程度。`,
      '职业': `以“${name}”为核心的内化职业能力、经验与胜任资格。`,
      '装备': `装备词条，说明“${name}”的当前状态、效果、持有者与可调用方式。`,
      '物品': `物品词条，说明“${name}”的数量、用途、所在位置和消耗或转让条件。`,
      '穿着': `穿着词条，说明“${name}”占用的槽位、外观、状态和对现实行动的影响。`,
      '社群角色': `社群角色词条，说明角色在某个社群中的身份、职责和关系位置。`,
      '阵营': `阵营词条，说明角色所属群体及其在群体中的立场。`,
      '人事归属': `组织归属词条，说明角色在某个组织中的部门、职位或成员关系。`,
      '性经验分类': `用于记录某一类亲密经历的累计情况与变化依据。`,
      '当前身体状态': `用于记录角色身体某个部位当前的状态、描述与更新时间。`,
      '身体原貌': `用于记录角色身体原貌的结构化描述。`,
      '盛装状态': `用于记录角色盛装外观和身体部位的修饰状态。`,
    };
    return typeText[kind] || `执行“${name}”相关行动时所需的理解、熟练度和稳定发挥能力。`;
  },
  ...
  rpgItemDetail(field, item) {
    ...
    const lines = [
      ...
      `类型: ${kind}`,
      `所属世界: ${field?.worldTag || lexicon?.worldTag || '公共'}`,
      `词条类型: ${field?.targetType || lexicon?.meta?.targetType || '角色'}`,
    ];
    if (obj?.part) lines.push(`部位: ${obj.part}`);
    if (obj?.status) lines.push(`状态: ${obj.status}`);
    if (obj?.slot || obj?.slotLabel) lines.push(`槽位: ${obj.slotLabel || obj.slot}`);
    if (obj?.clothing_position) lines.push(`穿戴位: ${obj.clothing_position}`);
    if (obj?.index != null) lines.push(`序号: ${obj.index}`);
    if (Array.isArray(obj?.tags) && obj.tags.length) lines.push(`标签: ${obj.tags.join('、')}`);
    if (Array.isArray(obj?.requiredSkills) && obj.requiredSkills.length) lines.push(`前置技能: ${obj.requiredSkills.join('、')}`);
    if (Array.isArray(obj?.requiredKnowledge) && obj.requiredKnowledge.length) lines.push(`前置知识: ${obj.requiredKnowledge.join('、')}`);
    if (Array.isArray(obj?.linkedStats) && obj.linkedStats.length) lines.push(`关联属性: ${obj.linkedStats.join('、')}`);
    if (Number(obj?.level) > 0) lines.push(`等级: lv.${obj.level}`);
    if (exp.current != null || exp.next != null) lines.push(`经验: ${Number(exp.current) || 0}/${exp.next === Infinity ? 'max' : (Number(exp.next) || '--')}`);
    if (obj?.description) lines.push(`描述: ${obj.description}`);
    if (obj?.reason) lines.push(`变化原因: ${obj.reason}`);
  ...
  rpgFieldDetail(field) {
    ...
    const lines = [
      `完整内容: ${rawValue || '未记录'}`,
      `说明: ${lexicon?.description || lexicon?.summary || field?.desc || this.fallbackDesc(field)}`,
    ];
    if (field?.pendingAiInit) lines.push('初始化: 否，当前为模板占位，待 AI 初始化。');
    if (field && Object.prototype.hasOwnProperty.call(field, 'initialMeeting')) {
      lines.push(`初始见面: ${Array.isArray(field.initialMeeting) ? field.initialMeeting.join('、') || '无' : field.initialMeeting}`);
    }
    ...
    if (field?.source) {
      lines.push(`来源: 初始 ${field.source.initial || 0} / 等级 ${field.source.level || 0} / 分配 ${field.source.allocated || 0} / NPC成长 ${field.source.npc || 0}`);
    }
    return lines.filter(Boolean).join('\n');
  },
  ...
  identityInfoFieldMeta(field = {}) {
    ...
    if (match(/职业|(^|[-_])job$/)) return { icon: '▣', section: 'core', tone: 'gold' };
    if (match(/世界|world_tag|(^|[-_])work$/)) return { icon: '◎', section: 'tag', tone: 'cyan' };
    ...
    if (match(/社群角色|阵营|factions/)) return { icon: '◍', section: 'list', tone: 'cyan', itemIcon: '•' };
  ...
  goalsPresentation(fields = []) {
    const rows = (fields || []).filter(Boolean).map((field) => {
      ...
      const meta = {
        ...
        short: { icon: '★', title: '近期目标', tone: 'cyan' },
        medium: { icon: '◆', title: '中期目标', tone: 'gold' },
    ...
    const cards = ['short', 'medium', 'long'].map((role) => {
      ...
      return {
        ...
        icon: row?.icon || (role === 'short' ? '★' : role === 'medium' ? '◆' : '♥'),
  ...
  intrinsicAbilityPresentation(fields = [], state = null) {
    ...
    const stats = this.intrinsicStatMeta().map((meta) => {
      ...
      const breakdownText = breakdown
        ? `初始${breakdown.initial} + 等级${breakdown.level} + 分配${breakdown.allocated}${breakdown.npc ? ` + 成长${breakdown.npc}` : ''}`
        : '';
      return {
        ...meta,
        field,
  ...
  itemEmoji(itemOrName = '') {
    ...
    if (/食|餐|饭|零食|面包|水果/i.test(n)) return '🍞';
  ...
  normalizeBodyFigurePickerItem(item = {}, target = this.bodyFigurePickerTarget || {}) {
    ...
    const currentPath = String(target.currentPath || '').trim();
    const active = Boolean(currentPath && String(item.path || '').trim() === currentPath);
    const boundOwnerName = boundOwnerId ? this.bodyFigureCharacterName(boundOwnerId, item) : '';
    return {
      ...item,
      active,
      boundOwnerName,
      disabled: Boolean(boundOwnerId && target.characterId && boundOwnerId !== target.characterId),
  ...
  async refreshBodyFigurePickerItems() {
    ...
    try {
      const currentPath = window.GameModules.bodyFigure?.getBindingPath?.(target.characterId, target.kind) || '';
      const choices = await window.GameModules.bodyFigure?.listFigureChoices?.(target.figureMeta, target.rows) || [];
      this.bodyFigurePickerItems = choices.map((item) => this.normalizeBodyFigurePickerItem(item, { ...target, currentPath }));
    } catch (err) {
  ...
  bodyProfilePresentation(field, sectionTitle = '', stateOverride = null) {
    ...
    const fallbackSilhouette = this.fallbackBodySilhouette(profile, meta, sectionTitle);
    const resolvedSilhouette = window.GameModules.bodySilhouette?.resolvePresentation?.(profile, meta, sectionTitle) || null;
    const silhouetteBase = resolvedSilhouette
      ? { ...fallbackSilhouette, ...resolvedSilhouette, src: fallbackSilhouette.src, remote: false }
      : fallbackSilhouette;
  ...
  fallbackBodySilhouette(profile = {}, meta = {}, sectionTitle = '') {
    ...
    const inlineSvg = key === 'male-youth'
      ? '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200" role="img" aria-label="Male youth silhouette"><g fill="#c8d4e6" stroke="none"><ellipse cx="50" cy="15" rx="13" ry="14"/><rect x="44" y="29" width="12" height="8" rx="2"/><path d="M34 38 Q50 34 66 38 L64 78 Q50 82 36 78 Z"/><path d="M23 42 Q18 58 20 84 L30 82 Q31 59 35 42 Z"/><path d="M77 42 Q82 58 80 84 L70 82 Q69 59 65 42 Z"/><path d="M38 78 L36 124 Q50 129 64 124 L62 78 Z"/><path d="M38 124 L35 194 L44 194 L48 136 L52 136 L56 194 L65 194 L62 124 Z"/></g></svg>'
      : '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200" role="img" aria-label="Female shoujo silhouette"><g fill="#c8d4e6" stroke="none"><ellipse cx="50" cy="16" rx="14" ry="15"/><rect x="44" y="30" width="12" height="8" rx="2"/><path d="M33 38 Q50 36 67 38 L65 80 Q50 84 35 80 Z"/><path d="M20 42 Q16 62 18 78 L28 76 Q30 56 32 44 Z"/><path d="M80 42 Q84 62 82 78 L72 76 Q70 56 68 44 Z"/><path d="M36 80 Q50 86 64 80 L60 108 Q50 112 40 108 Z"/><path d="M38 108 L36 138 Q50 142 64 138 L62 108 Z"/><path d="M36 138 L34 168 Q50 172 66 168 L64 138 Z"/><path d="M34 168 L32 194 Q42 198 50 198 Q58 198 68 194 L66 168 Z"/></g></svg>';
    ...
    return {
      ...
      src: 	`data:image/svg+xml;charset=UTF-8,${encodeURIComponent(inlineSvg)}`,
      remote: false,
      sectionTitle,
  ...
  bodySilhouetteInlineSvg(key = '') {
    const map = {
      'female-shoujo': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200" role="img" aria-label="Female shoujo silhouette"><g fill="#c8d4e6" stroke="none"><ellipse cx="50" cy="16" rx="14" ry="15"/><rect x="44" y="30" width="12" height="8" rx="2"/><path d="M33 38 Q50 36 67 38 L65 80 Q50 84 35 80 Z"/><path d="M20 42 Q16 62 18 78 L28 76 Q30 56 32 44 Z"/><path d="M80 42 Q84 62 82 78 L72 76 Q70 56 68 44 Z"/><path d="M36 80 Q50 86 64 80 L60 108 Q50 112 40 108 Z"/><path d="M38 108 L36 138 Q50 142 64 138 L62 108 Z"/><path d="M36 138 L34 168 Q50 172 66 168 L64 138 Z"/><path d="M34 168 L32 194 Q42 198 50 198 Q58 198 68 194 L66 168 Z"/></g></svg>',
      'male-youth': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200" role="img" aria-label="Male youth silhouette"><g fill="#c8d4e6" stroke="none"><ellipse cx="50" cy="15" rx="13" ry="14"/><rect x="44" y="29" width="12" height="8" rx="2"/><path d="M34 38 Q50 34 66 38 L64 78 Q50 82 36 78 Z"/><path d="M23 42 Q18 58 20 84 L30 82 Q31 59 35 42 Z"/><path d="M77 42 Q82 58 80 84 L70 82 Q69 59 65 42 Z"/><path d="M38 78 L36 124 Q50 129 64 124 L62 78 Z"/><path d="M38 124 L35 194 L44 194 L48 136 L52 136 L56 194 L65 194 L62 124 Z"/></g></svg>',
    };
    return map[String(key || '').trim()] || '';
  },

  bodySilhouetteDisplaySrc(silhouette = {}) {
    const raw = String(silhouette?.src || '').trim();
    if (!raw) return '';
    if (raw.startsWith('data:image/svg+xml')) return raw;
    const matched = raw.match(/body-silhouettes\/([^/?#]+)\.svg/i);
    const key = String(matched?.[1] || silhouette?.key || '').trim();
    const inlineSvg = this.bodySilhouetteInlineSvg(key);
    return inlineSvg ? `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(inlineSvg)}` : raw;
  },
