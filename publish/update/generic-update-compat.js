...
Object.assign(window.GameModules.updateRegistry, {
  ...
  genericToMetricUpdates(updates = [], type = 'emotion') {
    ...
    for (const item of Array.isArray(updates) ? updates : []) {
      ...
      const target = subject.characterId || subject.playerId || subject.id || item.target || 'player-self';
      const key = String(item.field || '').split('.').filter(Boolean).at(-1) || item.name;
      if (!key) continue;
      if (!grouped.has(target)) grouped.set(target, { target, subject, emotions: [], playerFeelings: [] });
      const reason = this.reasonObject(item);
  ...
  expandGenericForLegacy(result = {}) {
    const updates = Array.isArray(result.genericUpdates) ? result.genericUpdates : [];
    if (!updates.length) return result;
    const emotions = this.genericToMetricUpdates(updates, 'emotion');
    const feelings = this.genericToMetricUpdates(updates, 'feeling');
