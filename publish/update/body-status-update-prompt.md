...
确认玩家或角色当前身体部位状态发生稳定的性唤起或情欲变化时，返回 updateType:"body-status"。
...
- change.value 必须是对象：{ "partKey": "部位键名", "part": "部位中文名", "status": "情欲状态标签", "描述状态": "色情化感官短说明" }。
...
- reasons.trigger：写导致上述情欲状态变化的明确现实事件（如亲吻、抚摸、插入、言语挑逗、自慰、摩擦、暴露等），聚焦具体的性刺激行为或情境。