...
window.GameModules.playerSetupActions = {
  playerProfileLexiconFields() {
    ...
    const fields = [
      ...
      row('当前位置', window.GameModules.currentLocationField?.display?.(p) || p.currentLocation, '玩家当前所在位置，格式为“所在世界·所在势力·动态层级链·地图地点·详细位置”；其中倒数第2段直接作为电子地图节点名。'),
    ...
    try {
      ...
      try {
        ...
        if (err?.code === 'AUTH_REQUIRED') {
          const fallback = window.GameModules.playerSetupActions?.completePlayerSetupLocalFallback;
          if (typeof fallback === 'function' && options.forceAi !== true) {
            this.profileSetupBusy = false;
            await fallback.call(this, { ...options, skipAi: true });
            return;
          }
  ...
  normalizePlayerSetupBase(name, birthday) {
    ...
    return {
      ...
      appearance: String(p.appearance || '').trim().slice(0, 1000),
      preferences: String(p.preferences || '').trim().slice(0, 1000),
      personality: String(p.personality || '').trim().slice(0, 1000),
