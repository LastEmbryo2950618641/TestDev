...
function registerGameStore() {
  ...
  Alpine.store('game', {
    ...
    log: [], realWorldOpen: false, realWorldBusy: false, realWorldInput: '', realWorldFunctionOpen: false, realWorldFunctionView: 'menu', realWorldSceneTitle: '现实世界', realWorldLocationName: '', realWorldMap: window.GameModules.realWorldMap.defaultState({}), realWorldQuest: '确认手机异常与现实处境', realWorldStatus: '现实稳定', realWorldChoices: ['检查手机记录', '观察居住环境', '联系熟人确认', '暂时休息'], realWorldLog: [], realWorldlineState: { plots: [], pendingPlot: null }, realWorldProfileOpen: true, companyState: window.GameModules.companySystem.defaultState({}), bossState: window.GameModules.bossRecruitment.defaultBossState({}), calendarState: window.GameModules.calendarSystem.defaultCalendarState(), factionState: window.GameModules.factionSystem.defaultState({}), skillsState: window.GameModules.skillsApp.defaultState(), promptState: window.GameModules.promptTemplates.defaultState(), tokenStatsState: window.GameModules.tokenStats.defaultState(),
    nextId: 1,
    ragQuery: '',
    ragContext: '',
    memoryContext: '',
