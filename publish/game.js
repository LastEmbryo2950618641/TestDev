...
document.addEventListener('alpine:init', () => {
  ...
  Alpine.store('game', {
    ...
    log: [], realWorldOpen: false, realWorldBusy: false, realWorldInput: '', realWorldSceneTitle: '现实世界', realWorldQuest: '确认手机异常与现实处境', realWorldStatus: '现实稳定', realWorldChoices: ['检查手机记录', '观察居住环境', '联系熟人确认', '暂时休息'], realWorldLog: [], realWorldProfileOpen: true, companyState: window.GameModules.companySystem.defaultState({}), bossState: window.GameModules.bossRecruitment.defaultBossState({}), calendarState: window.GameModules.calendarSystem.defaultCalendarState(),
    nextId: 1,
    ragQuery: '',
    ragContext: '',
    memoryContext: '',
    ...
    async init() {
      ...
      document.getElementById('boot-fallback')?.remove(); this.initPromise = (async () => {
        try {
          ...
          this.initCompanySystem?.(); this.initBossRecruitment?.(); this.initCalendar?.();
        } catch (err) {
