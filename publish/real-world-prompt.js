...
window.GameModules.createRealWorldPrompt = async function createRealWorldPrompt(state, action) {
  ...
  const outputJson = JSON.stringify({
    sceneTitle: '现实场景标题', locationName: '具体地点名', parentLocationName: '上级地点名', locationDescription: '地点说明', mapNodes: [{ name: '子地点名', parentName: '上级地点名', description: '地点说明' }], elapsedSeconds: 60, thinking: '60到140字，概括现实推演依据，不写隐藏推理', narration: '以第二人称续写现实世界中的行动结果，180到360字，现实、克制、细节充分', status: '现实状态简述', quest: '新的现实目标', choices: ['处理现实事务', '联系某个人', '观察周围', '暂时休息'],
    lexiconUpdates: [{ worldTag: realWorld.label || '2026 现代都市现实世界', kind: '玩家设定/装备/物品/穿着', name: '词条名', value: '新值或对象', summary: '摘要', description: '说明', reason: '为什么现实行动导致该词条需要修改或新增' }],
  });
  return window.GameModules.promptTemplates.render('real-world-engine', {
    ...
    玩家资料: state.playerSetupSummary?.() || `姓名/代号：${state.playerName || '玩家'}`, 玩家属性: state.playerIdentitySummary?.() || '玩家本人属性尚未生成。', 公司系统: state.companyPromptContext?.() || '暂无公司系统词条。', 当前场景: state.realWorldSceneTitle || '现实世界', 当前地点: state.realWorldLocationName || map.current, 现实地图: map.lastText || window.GameModules.realWorldMap.render(map), 已知地点: (map.nodes || []).map((node) => `${node.parentId ? '子地点' : '根地点'}：${node.name}`).join('；') || map.current, 当前目标: state.realWorldQuest || '确认手机异常与现实处境', 最近记录: recent, 本次行动: action || '继续观察现实世界', 输出示例: outputJson,
  });
};
