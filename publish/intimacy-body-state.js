...
window.GameModules.intimacyBodyState = {
  ...
  fields(state = {}) {
    ...
    return [
      ...
      { key: 'sexualPartners', stateId: state.id || '', label: '经历人列表', kind: '性经历', value: partners.length ? partners : ['无'], raw: partners, initialMeeting: '无', desc: '已确认计入经历人数的对象列表，自动去重。', reason: values.intimacy?.reason || '默认未记录。', worldTag, targetType: p.isPlayer ? '非角色' : '角色', commonField: true },
      { key: 'sexualExperienceCount', stateId: state.id || '', label: '性经验总次数', kind: '角色卡', value: this.adultConfirmed(state) ? `${count}次` : '未确认成人，不自动更新', raw: count, initialMeeting: `${initial.sexualExperienceCount || 0}次`, desc: '成人虚构角色的抽象经历总次数；同一次经历可关联多个分类。', reason: values.intimacy?.reason || '默认未记录。', worldTag, targetType: p.isPlayer ? '非角色' : '角色', commonField: true },
