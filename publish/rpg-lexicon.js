...
window.GameModules.rpgLexicon = {
  ...
  defaultPromptInstruction(kind, name) {
    ...
    if (kind === '阵营') return this.compactPrompt('阵营角色叶子词条，名称与定义都必须写明阵营和角色两个字段', '加入退出阵营、住址或社会角色变化时才可改变。');
    if (kind === '势力地位') return this.compactPrompt('势力地位叶子词条，名称与定义都必须写明势力和地位两个字段，地位必须体现等级、职级、年级或职位', '加入退出势力、任免岗位、升降级、转部门或职级变化时才可改变。');
