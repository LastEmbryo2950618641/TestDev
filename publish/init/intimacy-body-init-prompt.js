...
const template = {
  ...
  fieldMeta: {
    sexualStatus: { label: '当前状态', kind: '性经历', desc: '成人虚构角色的性经历状态，如处女、非处女等。', reasonFallback: '默认未记录。' },
  ...
  fields() {
    return {
      ...
      bodyDescriptions: { defaultValue: this.clone(this.bodyDescriptions), meaning: '每个身体部位的默认露骨处女状态描述。' },
      ...
      bodyStatusDefaults: { defaultValue: this.clone(this.bodyStatusDefaults), meaning: '完整身体各部位状态初始化对象（露骨处女版）。' },
  ...
  editableFields() {
    ...
    return {
      intimacy: {
        meaning: '玩家或角色的亲密经历初始化，只保存中性元数据，默认处女。',
      ...
      bodyStatus: {
        meaning: '各身体部位当前状态初始化，保存露骨短状态和描述，默认处女。',
        ...
        fields: {
          ...
          status: { defaultValue: f.valueDefaults.defaultValue?.bodyStatus, meaning: '短状态，如处女、已破处、已开发、红肿、湿润等。' },
          description: { defaultValue: f.valueDefaults.defaultValue?.bodyDescription, meaning: '露骨的中性状态描述；可包含粉嫩、紧窄等形容词，但不要写过程描写。' },
  ...
  promptText() {
    return [
      '### 亲密与身体状态初始化字段模板（露骨版）',
      '只在现实推演正文明确支持初始化时填写；没有依据的字段保持缺省值或不返回。默认所有部位均为处女状态。',
