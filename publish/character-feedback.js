...
window.GameModules.characterFeedback = {
  ...
  prompt(store) {
    ...
    const metricJson = {
      emotions: [{ key: '只返回需要设定初始值的情绪名', value: 0, status: `${actor}当前具体表现`, reason: '具体成因' }],
      playerFeelings: [{ key: '只返回需要设定初始值的感觉名', value: 0, status: `${actor}对你产生的当前具体表现`, reason: '具体成因' }],
    };
    ...
    return `根据角色当前设定生成被操控后的内心反馈，并更新该角色对“被上线”的感觉。必须只返回合法JSON，不要Markdown。角色：${store.character.name}｜${store.character.role}｜${store.character.work}。年龄：${store.characterAge || '未知'}。操控方式：${store.controlMode}。当前正在发生：${store.entryCurrentAction || '未知'}。人物资料：${profile}。已有上线体验：上线次数=${experience.onlineCount}；当前被上线感觉=${experience.feeling}；适应度=${experience.adaptation}/100；体验摘要=${experience.summary}。固定数值：当前情绪=${JSON.stringify(store.emotions)}；对玩家感觉=${JSON.stringify(store.playerFeelings)}。固定维度含义：${metricDefs}。爱情阶段每约12.5分晋级：心动、爱恋、倾心、眷恋、深爱、执念、依存、相守。核心处境：对角色本人来说，身体是突然不受控制的；她不知道是谁在控制，也不知道控制来源，只能先感到自己的身体突然自己行动。上线后她像被困在身体里旁观外界，无法控制动作和发声，但视觉、听觉、嗅觉、味觉、触觉、疼痛、疲劳等身体感觉仍然能感受到。规则：mind 必须是角色自己的第一人称内心独白，不要旁白说明，不要写角色名字；intent 是角色自己下一步想要做什么，不是玩家行动选项；choices 必须根据当前角色、场景、上线状态、危险与目标生成4个玩家可执行行动，每个12字内，不能包含“放开控制”，不能固定套用默认选项；metricUpdates 是首次见面的初始数值，只返回需要由 AI 设定或解释的字段，没有信息、无需更改或沿用默认值的字段不要返回；返回的 metricUpdates 项只能写 key、value、status、reason，不要返回 delta/stage/description；value 是 0-100 的当前初始值，必须根据角色资料、当前处境、上线方式、已有经历和玩家首次接触方式推定；status 和 reason 必须站在第三者上帝/作者视角描述，用“你”指玩家，用“${actor}”指角色；禁止写“按角色背景推定”“根据上下文”“初始感觉”“目的未知”等提示词式空话。status 写当前表现，例如“${actor}看到你时心里扑通扑通，似乎是心动了。”；reason 写具体成因，例如“你第一次出现就控制了${actor}的身体，所以${actor}无法信任你。”或“你拯救了${actor}，外貌也符合${actor}的偏好，所以${actor}对你心动。”。controlFeeling 可参考这些状态：${this.feelingExamples.join('、')}；也可以根据角色状态自定义一个更贴切的词、短句或简短感受描述。请根据角色性格、年龄、身体状态、经历、记忆、当前处境、上线次数和适应度综合判断：上线次数多可能逐渐适应，但不是必然；有些角色会疑惑、警惕或冷静分析，有些会害怕、屈辱或愤怒，不要固定模板。字段若没有新信息、没有变化或不需要更改，可以省略；不要为了凑格式返回空字符串、0 或重复旧值。格式示例：${outputJson}`;
  },
