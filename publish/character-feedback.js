...
window.GameModules.characterFeedback = {
  ...
  prompt(store) {
    ...
    const metricItem = (key) => ({ key, value: 0, status: `${actor}对你产生的当前具体表现`, reason: '你做了什么或当前处境为什么让她/他如此' });
    const metricJson = {
  ...
  parse(text, fallback, store) {
    try {
      ...
      return {
        ...
        metricUpdates: window.GameModules.ai.normalizeInitialMetricUpdates(data.metricUpdates, fallback.metricUpdates, store),
