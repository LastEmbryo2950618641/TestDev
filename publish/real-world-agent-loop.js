...
window.GameModules.realWorldAgentLoop = {
  ...
  async buildConfiguredNarrationPrompt({ store, action, base, loaded, skills, materialSession = null, config = this.realConfig() }) {
    ...
    if (config.mode === 'story') return [
      ...
      '要求：使用第二人称“你”称呼玩家；玩家不是角色本人，而是操控/影响被操控者行动的存在；写出动作过程、环境变化、其他人物反应、被操控者身体与心理张力、直接结果；正文目标1000-1300个中文汉字；不要替玩家完成后续行动。',
    ].join('\n\n');
    return [
      '# 现实推演阶段2：只生成玩家可见正文',
      ...
      `基础上下文：\n${base}`,
      `已动态载入资料：\n${[loadedText, materialText].filter(Boolean).join('\n\n') || '无'}`,
      '要求：使用第二人称“你”；写出行动过程、环境变化、人物反应和直接结果；无论推演自由度是行动范围内还是AI自由推演，正文目标1000-1300个中文汉字；不要替玩家完成后续行动。',
    ].join('\n\n');
  },
