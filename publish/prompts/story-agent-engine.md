...
13. 行动提及、联系或影响具体人物时，先用 character.query.searchCharacterProfile 查询角色完整身份资料（角色卡会包含身份、关系、地点、状态、穿着、物品和情感数值；没有角色卡才返回介绍卡）；已经在基础上下文、作品资料或已动态载入资料中足够明确的人物，不要重复查询。
14. 如果基础上下文和已动态载入资料已经足够，不要为了形式请求资料，直接返回 context_done。
...
5. 正文风格、操控过程、角色心理、数值结算、角色卡更新都由阶段2和阶段3单独处理；本阶段不要预写。