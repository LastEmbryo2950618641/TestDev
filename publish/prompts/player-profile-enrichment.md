...
2. 格式为可变段数链式：`[势力层级链...]·地点·地点内位置`。前段势力层级按真实管辖结构写，段数不固定；至少 3 段。
3. **倒数第 2 段「地点」**必须是正式地图地点名，直接作为电子地图节点名，例如"锦苑小区3栋"、"星河云栈科技园B座"、"青石镇东市"、"王都白塔宫"。地图合并只认这一段。
4. **最后 1 段「地点内位置」**才允许写门牌、房间、工位、宿舍床位、宫殿内殿等内部位置；不再细分；不得把人物姓名拼进地点名。
5. 势力层级必须是实际管辖或控制结构。现代现实可用"中华人民共和国·四川省·成都市·武侯区"（多段）或"中华人民共和国·四川省成都市·武侯区"；异世界可用"银月王国·东境行省·青石郡"等对应世界观结构。
