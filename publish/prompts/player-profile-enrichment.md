...
2. 格式固定为：`所在世界·势力·层级1·层级2·地点|位置1·位置2·位置3`。
3. **`|` 前最后一段「地点」**必须是正式地图地点名，直接作为电子地图节点名，例如"锦苑小区3栋"、"星河云栈科技园B座"、"青石镇东市"、"王都白塔宫"。地图合并只认这一段。
4. **`|` 后位置链**才允许写门牌、房间、工位、宿舍床位、宫殿内殿等内部位置；不再细分；不得把人物姓名拼进地点名。
5. 所在世界与势力层级必须是实际世界与真实管辖或控制结构。现代现实请按层级分段写成"2026 现代都市现实世界·中华人民共和国·四川省·成都市·武侯区"这样的多段；不要把两个层级合并成同一段。异世界可用"银月王国·东境行省·青石郡"等对应世界观结构。
...
1. 职业是内化能力；workplace/position 用于合并展示为「人事归属：组织 / 职位」。
2. workplace 根据 refinedRole、地址和现实世界逻辑生成，表示可指认的公司、学校、部门、科室或机构；居住社区一般不要填成 workplace（居住社区属于社群角色侧）。
3. position 是玩家在该组织内的岗位、年级、职位、职级或成员身份，例如「软件工程师」「高三学生」「科室经理」。
4. **社群角色 vs 人事归属（含义）**：社群角色是软性圈子里的社会角色（家庭成员、居民、朋友圈位置等）；人事归属是组织编制/学籍/职级等可核对身份。二者构成要素分别是「圈子名+角色」与「组织名+职位」。资料中已出现且有依据的身份应尽量归入对应侧，不要因过严过滤而漏掉。
...

{
  ...
  "properties": {
    ...
    "currentLocation": {
      "type": "string",
      "minLength": 1,
      "description": "玩家当前所在位置，格式固定为'所在世界·势力·层级1·层级2·地点|位置1·位置2·位置3'；`|` 前最后一段'地点'必须是正式地图地点名，并直接作为电子地图节点名；`|` 后位置链才写房间、门牌、工位等内部位置。"
    },
    "refinedRole": {
      "type": "string",
      "minLength": 1,
...

{
  ...
  "currentLocation": "2026 现代都市现实世界·中华人民共和国·广东省·深圳市南山区·科兴科学园B4栋|1802室",
