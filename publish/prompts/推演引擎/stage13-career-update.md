...
已有职业档案时，本阶段只会在本轮出现职业相关变化后调用。调用后，你可以依据本轮事实完整编辑对应职业档案的所有业务字段，包括任职状态、单位或自由职业名称、职位、直系领导、路线、部门、地点、制度、收入、项目、任务、风险、规则、晋级路线、订单、成果、能力、擅长与名声资料。请输出变化后完整且真实的档案；需要清空的数组直接返回 `[]`，不要为了保留旧资料而虚构内容。自由职业合并或改名时仅必须保留已有卡片 `id`。
...
- 当前是否到达评绩效日期：{{是否到达评绩效日期}}；下一次评绩效日期：{{下一次评绩效日期}}。绩效日期本身不会触发本阶段；只有本轮已经出现职业相关变化时，才结合日期决定是否执行评绩效并把日期推到未来。贡献价值仍必须满足额外贡献条件。
