...
- 势力创建与势力字段补丁改由 Stage8 势力更新负责；本阶段不要输出势力总览、势力结构、组织能力。
...
- **角色卡完整性**：比较当前角色卡与已经加载的完整上下文；只要社群角色、人事归属、证书、称号或其他允许更新字段存在适用锚点，且当前值缺失、模糊、不完整、归类错误或与上下文冲突，就必须输出对应操作。缺少具体名称只是缺少细节，不是没有依据。
- 四类身份字段缺少具体细节时，依据本轮正文、已载入背景、世界观、年代、地区、年龄、教育或职业经历作最小充分推演；禁止与设定矛盾的扩张，也禁止“某公司”“未知学校”“相关机构”“普通职员”“初中生”“成员”等模糊占位。
...
- 角色卡数组项必须使用类型化 `field/op/target/value/delta` 合约；操作权限以注入的共享更新规则和角色卡 JSON 合约为准。禁止旧中文字段行、`change.mode`、upsert 和整组数组覆盖。
