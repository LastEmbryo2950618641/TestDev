...
- 资料请求格式：资料类型，查询动作，查询对象全称，世界或范围全称。
...
- Stage1 的“创建势力”只表示登记待建候选，不立即写库；可在请求中附带可推断的类型等基础信息。禁止因“本轮未打电话/未拜访/未改字段”而跳过登记候选。
...
- 输出前必须自检：行动/资料/角色卡中出现的现实组织/公司/学校等势力名，若已载入势力列表未收录，须在 materialRequests 写「势力查询，创建势力，势力名，类型」作为待建候选；不得因本轮未互动/无字段更新而跳过。
