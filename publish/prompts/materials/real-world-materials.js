...
window.GameModules.realWorldMaterials = {
  items: [
    ...
    { id: 'faction-archive-search', title: '势力资料库搜索', size: 'medium', maxChars: 1800, skill: 'faction.query', method: 'searchFactionArchive', paramsHint: { keyword: '势力、组织、部门或事件关键词' }, when: '需要读取某个国家、公司、学校、家庭、组织或部门的旧档案记录。' },
    ...
    { id: 'worldline-index', title: '世界线清单', size: 'small', maxChars: 1400, skill: 'realworld.history.query', method: 'listWorldlineIndex', paramsHint: {}, when: '需要先了解正在记录与已归纳现实世界线有哪些记录、情节、时间段和关键词。' },
    { id: 'worldline-keyword-search', title: '按关键词查询世界线资料', size: 'medium', maxChars: 1800, skill: 'realworld.history.query', method: 'searchWorldlineByKeyword', paramsHint: { keyword: '人物/地点/事件/物品/组织关键词' }, when: '需要根据关键词加载具体世界线记录或归纳情节资料。' },
    { id: 'worldline-time-search', title: '按时间查询世界线资料', size: 'medium', maxChars: 1800, skill: 'realworld.history.query', method: 'searchWorldlineByTime', paramsHint: { time: '日期、时间段或时间关键词' }, when: '需要根据发生时间、当天、昨天、某月某日等线索加载具体世界线资料。' },
    { id: 'worldline-plots', title: '已归纳情节目录', size: 'medium', maxChars: 1600, skill: 'realworld.history.query', method: 'listWorldlinePlots', paramsHint: {}, when: '只需要查看已归纳现实情节目录。' },
    ...
    { id: 'wechat-skills', title: '微信可操作技能清单', size: 'small', maxChars: 1000, skill: 'wechat.query', method: 'listWechatSkills', paramsHint: {}, when: '角色思念、联系玩家、发送当前或过去微信消息前，确认微信可执行操作。' },
    { id: 'wechat-contacts', title: '微信联系人清单', size: 'small', maxChars: 1000, skill: 'wechat.query', method: 'listContacts', paramsHint: {}, when: '需要确认角色是否在玩家微信里、联系人ID、关系和未读情况。' },
    { id: 'wechat-thread', title: '微信会话片段', size: 'medium', maxChars: 1600, skill: 'wechat.query', method: 'getThread', paramsHint: { contactId: '联系人id或角色id', count: 8 }, when: '角色思念事件准备用微信联系玩家，需要查看最近消息口吻与上下文。' },
    { id: 'wechat-send-now', title: '主动发送当前微信消息给玩家', size: 'small', maxChars: 900, skill: 'wechat.message.incoming', method: 'sendIncomingNow', paramsHint: { contactId: '联系人id或角色id', text: '消息内容' }, when: '思念角色在当前时刻主动给玩家发微信；在 final.wechatActions 中执行。' },
    { id: 'wechat-send-past', title: '主动在过去发送微信消息给玩家', size: 'small', maxChars: 900, skill: 'wechat.message.incoming', method: 'sendIncomingPast', paramsHint: { contactId: '联系人id或角色id', text: '消息内容', timeIso: '过去时间ISO' }, when: '回溯错过的思念触发，写入过去未读微信；在 final.wechatActions 中执行。' },
