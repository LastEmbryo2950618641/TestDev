...
window.GameModules.realWorldMaterials = {
  items: [
    ...
    { id: 'faction-list', title: '势力列表（含ID与组织架构）', size: 'medium', maxChars: 3200, skill: 'faction.query', method: 'listFactions', paramsHint: { world: '世界名' }, when: '正文前默认应加载；确认全部势力名/ID 与各自组织架构。' },
    ...
    { id: 'faction-field', title: '按势力ID/字段读取最新一条', size: 'small', maxChars: 1600, skill: 'faction.query', method: 'getFactionField', paramsHint: { world: '世界名', id: '势力ID', panel: 'ideology|economy|politics|military|diplomacy|territory', field: '字段名如core/institutions' }, when: '正文前可按势力ID+面板+字段只取最新局部数据（国体/经济等）。' },
    ...
    { id: 'entity-list', title: '实体状态列表', size: 'small', maxChars: 1400, skill: 'entity.query', method: 'listEntities', paramsHint: { world: '世界名', keyword: '可选实体关键词', limit: 12 }, when: '行动涉及具体物品、容器、设备、团体、线索、动物、车辆，或需要确认当前可追踪实体有哪些。' },
    { id: 'entity-search-one', title: '按指认特征查询实体当前态', size: 'small', maxChars: 1200, skill: 'entity.query', method: 'searchEntityOne', paramsHint: { world: '世界名', keyword: '实体名或关键词', owner: '归属/持有人可选', location: '地点可选', container: '容器可选', usage: '用途可选' }, when: '需要确认某个实体当前在哪里、归谁、状态如何；例如我的钥匙、某个包、某个小团体。' },
    { id: 'entity-history', title: '按锚点查询实体历史状态链', size: 'medium', maxChars: 1800, skill: 'entity.query', method: 'searchEntityHistory', paramsHint: { world: '世界名', keyword: '实体名或历史关键词', owner: '归属可选', location: '地点可选', container: '容器可选', historyLimit: 8 }, when: '当前态不足以区分同名实体，需要根据过去状态链、历史动作、容器、归属和时间锚点确认是哪一个实体。' },
    ...
    { id: 'wechat-send-now', title: '主动发送当前微信消息给玩家', size: 'small', maxChars: 900, skill: 'wechat.message.incoming', method: 'sendIncomingNow', paramsHint: { world: '世界名', contactId: '联系人id或角色id', text: '消息内容', intentChain: { cause: '起因', process: '过程', result: '结果', whyPlayer: '找主角原由' } }, when: '已是微信好友的角色在当前时刻主动给玩家发微信；在 final.wechatActions 中执行，必须带完整 intentChain。未通过好友申请前禁止使用。Inbox 已准备 wechat 来信时不要重复。' },
    { id: 'wechat-send-past', title: '主动在过去发送微信消息给玩家', size: 'small', maxChars: 900, skill: 'wechat.message.incoming', method: 'sendIncomingPast', paramsHint: { world: '世界名', contactId: '联系人id或角色id', text: '消息内容', timeIso: '过去时间ISO', intentChain: { cause: '起因', process: '过程', result: '结果', whyPlayer: '找主角原由' } }, when: '已是好友时回溯错过的未读微信；必须带完整 intentChain。' },
    { id: 'wechat-friend-request', title: '发起微信好友申请', size: 'small', maxChars: 900, skill: 'wechat.friend.request', method: 'requestWechatFriend', paramsHint: { world: '世界名', name: '申请人姓名', characterId: '角色id可选', reason: '申请理由', relation: '关系', intentChain: { cause: '起因', process: '过程', result: '结果', whyPlayer: '找主角原由' } }, when: '对方尚非微信好友但提出加微信；在 final.wechatActions 写 action:requestWechatFriend，必须带完整 intentChain。只产生待处理申请，禁止直接加为好友。' },
