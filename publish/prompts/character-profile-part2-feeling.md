...
Output Format：仅输出严格纯粹的紧凑 application/json。不要使用 Markdown 代码块包裹，不要输出代码围栏标记，不要 Pretty-print，不要换行缩进，不要输出任何解释、注释或额外文本。

Rules：

1. Schema 锁定：必须严格匹配下方字段定义，禁止新增未定义的 Key，禁止遗漏任何 required 字段。
2. 类型铁律：`feeling.emotions.*.value` 是 integer(0-100)。
3. 语法红线：严禁尾随逗号、额外大括号、匿名对象、数组包裹或对象套对象。`emotions` 内只能直接放 12 个固定键值对。
4. Key 顺序：严格按 `name` → `feeling` 顺序输出。`emotions` 内严格按 cold → fear → worry → joy → tension → anger → shame → sadness → curiosity → numbness → jealousy → despair 输出。
5. 生成到 `despair` 后立刻关闭 `despair`、`emotions`、`feeling` 和顶层对象，不要再添加任何字段或符号。
...
3. `status` 必须是 12-50 个汉字的短句，描述角色当前对该情绪的程度状态。必须结合角色本身性格、当前处境与过去经历来写，不得使用固定句式模板，不得只写抽象性格词。
4. `reason` 必须是 12-50 个汉字的短句，写形成该数值的具体原因。对于非零值，说明为什么该情绪会达到当前程度；对于零值，说明为什么角色完全没有此情绪。必须结合角色动机、处境、性格与过去经历，不得使用固定句式模板。
...
9. 禁止在 `numbness` 后额外开启 `{}` 包裹 `jealousy` 和 `despair`；`jealousy`、`despair` 必须与 `cold` 等字段同级。
