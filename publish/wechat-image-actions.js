...
window.GameModules.wechatImageActions = {
  appendWechatPendingImageMessage(characterId, state = {}, contact = {}, imageIntent = {}) {
    this.appendWechatMessage(characterId, { side: 'other', name: state?.profile?.name || contact.name, mark: (state?.profile?.name || contact.name || '').slice(0, 1), text: '', characterId, imagePending: true, imageStatus: 'pending', imageIntent });
  },
  ...
  async buildWechatImageTags(msg = {}) {
    ...
    const prompt = await window.GameModules.promptTemplates.render('wechat-image-prompt-collect', {
      ...
      自拍意图: [msg.imageIntent?.reason || '', msg.imageIntent?.tagsHint || ''].filter(Boolean).join('\n') || '发送一张当前自拍照',
    });
