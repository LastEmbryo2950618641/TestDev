...
window.GameModules.inferenceWorkPerformanceStageUpdate = {
  ...
  mergeFreelanceProfile(existing = {}, incoming = {}) {
    const pickArray = (next, prev) => Array.isArray(next) && next.length ? next : (Array.isArray(prev) ? prev : []);
    const reputationNext = incoming.reputationTitle || {};
    const reputationPrev = existing.reputationTitle || {};
    return {
      ...existing,
      ...incoming,
      ...
      reputationTitle: {
        title: reputationNext.title || reputationPrev.title || '',
        current: Number(reputationNext.current || reputationPrev.current || 0),
        max: Number(reputationNext.max || reputationPrev.max || 0),
        nextTitle: reputationNext.nextTitle || reputationPrev.nextTitle || '',
        review: reputationNext.review || reputationNext.evaluation || reputationPrev.review || reputationPrev.evaluation || '',
      },
      abilities: pickArray(incoming.abilities, existing.abilities),
      orders: pickArray(incoming.orders, existing.orders),
      works: pickArray(incoming.works, existing.works),
      currentProjects: pickArray(incoming.currentProjects, existing.currentProjects),
      currentTasks: pickArray(incoming.currentTasks, existing.currentTasks),
      risks: pickArray(incoming.risks, existing.risks),
      rules: pickArray(incoming.rules, existing.rules),
      openings: pickArray(incoming.openings, existing.openings),
      promotionRoutes: pickArray(incoming.promotionRoutes, existing.promotionRoutes),
    };
  },
  ...
  async runAfterSettlement({ store, action, narration, updates, logId, config, loop }) {
    ...
    loop?.patchConfiguredSettlementThinking?.(store, logId, reviewDue
      ? `Stage13：职业生涯已到评绩效日期 ${stats.nextPerformanceReviewAt}，强制要求 AI 评绩效并同步职业档案。`
      : 'Stage13：同步职业档案、上班状态、领导评价、员工评价与贡献价值。', {
      ...config,
    ...
    console.log('[职业生涯调试] Stage13 request', { reviewDue, nextPerformanceReviewAt: stats.nextPerformanceReviewAt, logId });
    let raw = '';
    try {
