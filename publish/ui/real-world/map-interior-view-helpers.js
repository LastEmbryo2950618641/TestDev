...
window.GameModules.ui.realWorld.mapInteriorViewHelpers = {
  ...
  roomResidentsLabel(room = {}) {
    const interior = window.GameModules.realWorldMapInterior;
    const label = interior?.roomOccupancyLabel?.(room);
  ...
  interiorFloorRoomCountText(floor = {}) {
    ...
    const roomNumbers = rooms
      .map((room) => String(room?.number || room?.name || '').trim())
      .filter(Boolean)
      .slice(0, 3);
    return roomNumbers.length ? `${roomCount} 个空间 · ${roomNumbers.join('、')}` : `${roomCount} 个空间`;
  },
