  ...
  playerProfile(store) {
    ...
    return {
      ...
      playerHome: this.lines([
        ['具体地址', p.refinedCity || p.city], ['居住状态', p.refinedLivingStatus || p.livingStatus], ['财富等级', p.wealthTier], ['当前财富', p.wealthAmount ? `${Number(p.wealthAmount).toLocaleString('zh-CN')}元` : '0元'], ['财富来源', p.wealthSource], ['父母状态', p.parentStatus || p.parents], ['父母去世原因', p.parentDeathCause || '无'],
      ]),
