...
window.GameModules.loadingActions = {
  ...
  ensureGameplayAssetsReady() {
    if (this._gameplayAssetsReady) return Promise.resolve();
    if (this._gameplayAssetsLoadingPromise) return this._gameplayAssetsLoadingPromise;
    const loader = window.GameModules.assetLoader;
    if (!loader) return Promise.resolve();
    this._gameplayAssetsLoadingPromise = loader.ensureGameplayReady(this).then(() => {
      window.GameModules.remergeGameStore?.();
      this._gameplayAssetsReady = true;
    }).finally(() => {
      this._gameplayAssetsLoadingPromise = null;
    });
    return this._gameplayAssetsLoadingPromise;
  },
