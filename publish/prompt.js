...
window.GameModules.createSystemPrompt = async function createSystemPrompt(state, action) {
  ...
  const outputJson = JSON.stringify({
    ...
    metricUpdates: { emotions: [{ key: '只返回本回合需要变化或解释的情绪名', delta: 0, status: '变化后的状态含义', reason: '导致变化的原因' }], playerFeelings: [{ key: '只返回本回合需要变化或解释的感觉名', delta: 0, status: '变化后的状态含义', reason: '导致变化的原因' }] },
