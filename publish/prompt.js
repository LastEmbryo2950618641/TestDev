...
3. mind 必须写成${character.name}自己的第一人称内心独白，参考她当前年龄、身体状态、身份经历、人格、记忆和剧情处境；不要写成外部动作、旁白、设定说明或“观察四周”这类行动描述。
4. online 时 mind 要体现身体主导权被夺走后的合理内心反应；具体可以是恐惧、震惊、麻木、愤怒、抗拒、计算、求生等，由角色设定和当前处境决定，不要固定模板，也不要无依据地过度镇定。禁止写“${character.name}感到……”“内心反应取决于……”这类旁白说明。
...
必须只返回合法 JSON，不要 Markdown，不要代码块。格式：
{
  ...
  "mind":"${character.name}自己的第一人称内心独白，60字内；像‘怎、怎么回事，我的身体不听使唤了……’这样直接写她心里冒出的念头；online 时只写被夺取身体主导权后的心理反应，不写行动",
