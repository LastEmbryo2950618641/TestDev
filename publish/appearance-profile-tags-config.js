...
window.GameModules.appearanceProfileTags = {
  ...
  bodyChangeFactGuide() {
    return [
      ...
      '',
      '### 三项判定（须全部满足）',
      '1. **正文确认**：正文或已结算事实中有可核对依据——量体/读数/体检/尺码/影像/他人目检记录/已发生的伤害或治疗结果/与系统已确认状态（如结算、档案计数）一致等。不合格：玩家或角色单方面断言、「感觉/似乎/好像」、无验证的台词。',
      '2. **属于身体外观**：变化必须落在真实身体当前可见状态，而非纯衣着、纯姿势、纯镜面/打光/化妆造成的视觉错觉。',
      '3. **可落成当前外观描述**：只要已经形成可写进 tags / description / meta 的当前外观差异即可更新；不再要求必须“永久”或必须“跨档”，但仍需避免完全不可观察的细碎脑补。',
      '',
      '### 变化来源（并列示例，非穷尽、非优先级）',
      '- 时间与发展：久别实测、发育完成、年龄带来的整体/meta 变化',
      '- 体态与 meta：体重秤/体检支持的 weight、figure、overall 变化',
      '- 造型与毛发：剪染烫、修剪、留长、剃净等已在当前状态形成结果的变化',
      '- 训练与劳损：锻炼或劳作导致的肌肉、线条、肤况、姿态观感变化',
      '- 伤损与修复：事故、手术、疤痕、淤痕、抓痕、咬痕、压痕、肿胀等已在正文发生且当前可见',
      '- 亲密/性相关后的身体观感：潮红、湿润、水迹、体液痕迹、局部充血、外露后可见状态变化等；**只要正文确认且当前可见**，即可更新对应部位',
      '- 状态未落到外观：只有心理感受、快感、疼痛、疲惫等主观体验，但未形成可观察体貌 → 不写',
  ...
  stage5GateTriggerGuide() {
    return [
      ...
      '- weight / figure / overall / height：须有客观依据，并且当前外观或数值已能确认差异；height 仍极少',
      '',
      '### 部位（updateScope: parts）',
      '- 按证据指向**当前已形成外观变化**的部位更新，不限于某一类剧情',
      '- 有离散档位的 tags（如部分量级、figure）可把“跨档”视为强证据，但不再是唯一入口；无档位区分的 tags 只要已进入明确可描述的新状态，也可更新',
      '',
      '## 一律不更新 bodyProfile',
      '- 无事实依据；仅感受/断言/playersay',
      '- 纯衣着、纯姿势、纯镜面/打光/化妆造成的视觉差',
      '- 只有心理感受或身体感觉，但没有形成可观察外观',
      '- 完全不可确认、不可落描述的细碎脑补',
    ].join('\n');
  },
