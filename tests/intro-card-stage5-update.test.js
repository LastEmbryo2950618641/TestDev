...
async function run() {
  ...
  assert.strictEqual(prompts.length, 2, 'Stage5 should render two AI prompts');
  assert.strictEqual(configs.length, 2, 'Stage5 should make two AI requests');
  assert.strictEqual(configs[0].kvCacheSession.id, 'post-body-fork', 'Stage5-1 should reuse post-body KV config');
  assert.strictEqual(configs[1].kvCacheSession.id, 'post-body-fork', 'Stage5-2 should reuse post-body KV config');
  assert.ok(String(prompts[0] || '').includes('Stage5-1 介绍卡建卡'));
  assert.ok(String(prompts[1] || '').includes('Stage5-2 介绍卡更新'));
