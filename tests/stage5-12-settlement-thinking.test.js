...
assert.ok(loop.includes("if (/^stage(?:[2-9]|1[0-2])$/u.test(phase)) {"));
assert.ok(loop.includes("return /^(stage(?:[4-9]|1[0-2]))$/u.test(phase);"));
assert.ok(loop.includes("match(/^Stage\\s*(1[0-2]|[1-9])"));
...
console.log('stage5-12 settlement thinking wiring verified');