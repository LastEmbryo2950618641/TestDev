...
hierarchy：层级，tree 表示会产生子词条的树词条，leaf 表示不再产生子词条的叶子词条
...
- `hierarchy` 只用于结构与界面展示：`tree` 表示会产生其他词条，例如知识储备、技能等级、职业等级；`leaf` 表示具体知识、具体技能、具体职业、属性或玩家设定等末端词条。
- AI 批量修改词条时只能修改传入字段；未传字段必须保持原值。
- AI 可以修改 `summary`、`description`、`value`、`aliases`、`related`、`meta`，但不能修改 `promptInstruction`、`nameAiGenerated`、`valueAiGenerated`、`changeMode`、`hierarchy`。
