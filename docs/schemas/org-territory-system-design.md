...
- **势力**与**社群**的区别，不在于是否使用这套结构，而在于 AI 基于上下文、已固化事实、现实常识 / 作品设定常识，对该主体是否已形成可持续组织主体性的判断。
- 五大基础 `ideology`、`economy`、`politics`、`military`、`diplomacy` 是重要证据，但不是唯一机械条件；资料未展开不等于主体不存在。
- 已知现实主权国家、上下文确认的架空主权体、明确具备持续组织主体性的组织，可直接归为**国家 / 势力**；即使五面板为空，也只表示“资料未展开”。
- 单纯自称、玩笑、宣传、角色扮演、过家家式“建国”，不得仅凭 `type=国家` 归为国家；应记录为 `claim` 或社群型 org，除非后续推演形成事实政权 / 稳定组织主体性。
- 一旦某对象曾经完整具备势力标准并被确认为势力，后续即使衰败、残缺、失能，也**不会退化回社群**，只会继续作为势力走向分裂、解体、合并或消亡。

#### 5.1.1.0 分类字段（AI 分类优先）

`classification` 为玩家一级显示与推演语义分类，取值：

| 值 | 含义 | 判断依据 |
| --- | --- | --- |
| `country` | 国家 / 主权体 | AI 根据现实常识、作品设定或上下文确认其具有主权体资格；通常配合 `sovereign=true` 或 `orgDomain=country`。 |
| `faction` | 势力 | 已形成稳定目标、成员边界、规则 / 领导、资源调度、对外行动等可持续组织主体性。 |
| `community` | 社群 | 组织事实存在或可建 stub，但尚未形成完整持续组织主体性。 |
| `claim` | 自称 / 宣称体 | 有国家、独立、势力名号等宣称，但上下文或常识不足以确认其为真实国家 / 势力。 |

补充规则：

1. `classification` 由 AI / audit / Stage4 优先写入；代码只做保守派生与兜底。
2. `maturityClass` 为兼容旧 UI 的派生字段：`country` 与 `faction` 映射为 `faction`，`community` 与 `claim` 映射为 `community`。
3. `type` 只是自称或显示类型，不是分类真源；`type=国家` 不得单独触发 `classification=country`。
4. `sovereign=true` 或 `orgDomain=country` 表示结构层主权体，应默认显示为 `country`；但普通人的自称国家不得写这两个字段。
5. 中国、美国、日本等已知现实国家可由 AI 基于模型常识直接判为 `country`，不需要当前上下文补齐五面板。
...
| 类型 | 判断标准 | 典型特征 | 存储方式 |
| --- | --- | --- | --- |
| 势力 | AI 基于上下文、常识与已固化事实判断其已具备持续组织主体性；五大基础是重要证据 | 已具备完整主体性，能够以完整组织身份持续存在与行动 | org registry + `membership` + `overviewPanels` |
| 社群 | 与势力使用同构 schema，但 AI 判断其尚未形成持续组织主体性 | 可能已有组织者、规则、资源、行动力，但整体仍未形成完整势力 | org registry + `membership` + `overviewPanels` |

**因此，社群不是另一套简化实现，而是同一套实现下的“未完成势力”。**

- 社群可以已经拥有组织者、行动网络、局部资源，甚至部分政治或军事能力。
- 但只要 AI 根据上下文与常识判断它尚未形成持续组织主体性，它在分类上就仍是社群。
- 势力则意味着其目标、边界、规则 / 领导、资源调度、对外行动等已经进入可持续、可更新、可被推演追踪的状态。
...
| 正文明确五大基础已经全部建立，或 AI 根据上下文判断势力化符号已经足够稳定 | 势力 | 此时从社群升级为势力 |

**“中兴会”这类灰区对象的处理规则**：

1. 五大基础缺项时通常仍是社群，但若 AI 根据现实常识、作品设定或上下文确认它已是成熟组织主体，可直接归为势力 / 国家。
2. 只要 AI 判断势力化符号已经足够稳定，或五大基础全部补齐并被推演确立，它就升级为势力，即使它仍非法、隐秘、地下化。
