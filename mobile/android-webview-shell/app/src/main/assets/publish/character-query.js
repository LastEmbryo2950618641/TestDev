...
window.GameModules.characterQuery = {
  ...
  stateText(state = {}, fallbackWorld = '', maxChars = 0) {
    ...
    const rows = [
      ...
      this.line('年龄/生日', [profile.age, profile.birthday].filter(Boolean).join(' / ')),
      this.line('职业', profile.job || this.valueText(profile.professions)),
      ...
      this.line('社群角色', this.listText(profile.factions, 8)),
      this.line('人事归属', this.listText(profile.memberships, 8)),
      ...
      this.line('穿着', this.listText(profile.wearingItems || profile.wearing, 12)),
      this.line('物品', this.listText(profile.items, 12)),
      this.line('技能', this.listText(profile.skills, 10)),
      this.line('知识', this.listText(profile.knowledge, 8)),
      this.line('上线体验', this.valueText(profile.control_experience)),
