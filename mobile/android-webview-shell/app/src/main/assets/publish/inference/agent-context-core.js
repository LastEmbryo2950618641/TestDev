...
window.GameModules.realWorldAgentContextParts.core = {
  ...
  buildLoadedText(items = []) {
    ...
    return items.map((item, index) => {
      ...
      if (item?.unlimited === true || this.isRoleCardMaterial(item)) {
        return `### 资料${index + 1}｜${item.title}\n${raw}`;
      }
      const max = Number(item?.max);
      const body = Number.isFinite(max) && max > 0 ? this.limit(raw, max) : this.limit(raw, 1600);
      return `### 资料${index + 1}｜${item.title}\n${body}`;
    }).join('\n\n');
  },
  ...
  // 角色卡正文已由 characterQuery.stateText 结构化产出；这里只去协议污染后整卡放行，不限长度、不再字段白名单。
  loadedRoleCardRoutingSummary(item = {}) {
    const text = this.redactPromptPollution(item?.text || '');
    const lines = text
      .split(/\r?\n/u)
      .map((line) => line.trim())
      .filter((line) => line && !/undefined[:：]/iu.test(line));
    return lines.join('\n') || String(text || '').trim();
  },
  ...
  loadedNarrationSummary(items = []) {
    if (!items.length) return '无';
    // 非角色卡：只留可写进正文的空间/在场/通信事实（挡工具回执噪音），并限长。
    // 角色卡：stateText 已是可用资料，只去协议污染后整卡放行，不限长度、不再字段白名单。
    ...
    return items.map((item, index) => {
      ...
      const isRoleCard = this.isRoleCardMaterial(item) || /资料类型[:：](?:完整角色卡|介绍卡)/u.test(text);
      const lines = text
        .split(/\r?\n/u)
        .map((line) => line.trim())
        .filter((line) => line && !/undefined[:：]/iu.test(line) && !protocolKeywords.test(line))
        .filter((line) => (isRoleCard ? true : factKeywords.test(line)));
      if (isRoleCard) return lines.length ? `资料${index + 1}：${title}\n${lines.join('\n')}` : '';
      const clipped = this.limit(lines.slice(0, 10).join('\n'), 700);
      return clipped ? `资料${index + 1}：${title}\n${clipped}` : '';
    }).filter(Boolean).join('\n') || '无';
  },
